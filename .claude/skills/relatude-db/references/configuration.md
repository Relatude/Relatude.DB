...
|---|---|---|
| `AssemblyNameReference` | `Assembly.Load(Reference)` — or the entry assembly when `Reference` is null — then adds every type in `Namespace` | **works** |
| `TypeNameReference` | `Type.GetType(Reference)`, adding that type and everything it references | **works** |
| `JsonFile` | Reads a serialised `Datamodel` from `Reference` via the IO provider named by `FileIO` | **works** |
| `AssemblyFileReference` | — | throws `NotImplementedException` |
| `TypeNameFileReference` | — | throws `NotImplementedException` |
| `CSharpCodeFile` | — | throws `NotImplementedException` |

Only the first three exist in the current build. A failing source throws `Failed to load datamodel source {id}` and the container does not open.
...
Off by default, on both paths. When **off**, a plain node-typed property with no relation declaration becomes a `Reference`/`References` property. When **on**, such properties are turned into auto-created relations instead — the old behaviour. Turn it on only to keep an existing model working; new models should declare relations explicitly.
