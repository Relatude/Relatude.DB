...
|---|---|---|
| `CompiledTypes` | `Assembly.Load(Reference)` — or the entry assembly when `Reference` is null — then adds every type in `Namespace`. Called `AssemblyNameReference`, then `TypeReference`, before September 2026; both still read, and a settings file carrying one is rewritten to `CompiledTypes` on load. The single-type `TypeNameReference` kind was removed then | **works** |
| `RuntimeTypes` | Model files on disk read at open: a serialised `Datamodel` from `Filepath` (file or folder, default `Models/Json`), or from `Reference` via the IO provider named by `FileIO`. Called `JsonFile`, then `TextFiles`, before September 2026. A `TextFiles` source could also hold `.cs` files compiled at open, chosen by `FileFormat`; that kind is gone, and a source of any of the old file kinds is **dropped from the settings file when it is loaded** | **works** |

`SourceCodePath` (CompiledTypes only) names the folder with the C# files so the admin UI model editor can write to them; `GenerateModelFile` on top hands that folder to the editor, which deletes and regenerates every file in it at activation (asking first when a file lacks the `// <auto-generated>` marker). A failing source throws `Failed to load datamodel source {id}` and the container does not open.
