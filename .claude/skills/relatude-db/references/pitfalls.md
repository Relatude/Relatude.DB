...
- **Datamodel source namespaces match exactly, not by prefix.** The kinds are `CompiledTypes` and `RuntimeTypes`; the older names `TypeReference`/`AssemblyNameReference` and `TextFiles`/`JsonFile`/`CSharpCodeFile` still read, `TypeNameReference` does not. A settings file's `TypeReference` is rewritten to `CompiledTypes` on load, and its file-based sources (and the removed `FileFormat` key) are dropped — a model kept in files has to be added again as a `RuntimeTypes` source.
