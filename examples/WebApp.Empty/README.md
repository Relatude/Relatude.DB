...
React client in `Client/`. Generated by `relatude new WebApp.Empty` (project type `csharp_web_react_ts`,
the default; `relatude new --list-types` describes the other types, such as `csharp_web_mvc` for a
server-rendered content website in one project).
