...
public class SqliteGuidArrayIndex : PersistedGuidArrayIndexBase {
    readonly SqliteIndexStore _store;
    readonly string _tableName;
    public SqliteGuidArrayIndex(SetRegister sets, SqliteIndexStore store, string indexId, string tableName, string friendlyName, bool justCreated)
    ...
    protected override IEnumerable<KeyValuePair<int, Guid[]>> ReadAllPersisted() {
        ...
        while (reader.Read()) yield return new(reader.GetInt32(0), JsonSerializer.Deserialize<Guid[]>(reader.GetString(1))!);
    }
    protected override void PersistAdd(int nodeId, Guid[] value) {
