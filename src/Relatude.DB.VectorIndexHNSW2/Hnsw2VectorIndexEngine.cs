...
/// <summary>
/// Semantic index engine backed by <see cref="Hnsw2VectorIndex"/>, one index folder per semantic
...
public class Hnsw2VectorIndexEngine : ISemanticIndexEngine {
    ...
    readonly Hnsw2VectorIndexOptions _defaults;
    readonly Dictionary<string, Hnsw2VectorIndex> _indexes = [];
    public Hnsw2VectorIndexEngine(string baseIndexFolderPath) : this(baseIndexFolderPath, null) { }
    public Hnsw2VectorIndexEngine(string baseIndexFolderPath, Hnsw2VectorIndexOptions? defaultOptions) {
    ...
    public ISemanticIndex OpenSemanticIndex(SetRegister sets, string id, string friendlyName, AIEngine ai, Action<string>? log) {
        ...
        var index = new Hnsw2VectorIndex(sets, id, friendlyName, folder, ai, cloneDefaults(), log);
        _indexes[id] = index;
        return index;
    }
    // every index gets its own copy so runtime tuning of one (memory budget, accuracy) stays local to it
    Hnsw2VectorIndexOptions cloneDefaults() => new() {
