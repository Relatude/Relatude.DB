...
/// <summary>Tuning knobs for <see cref="Hnsw2VectorIndex"/>. All sizes are in bytes.
///
/// <para>Two knobs decide where the index sits between speed and footprint.
...
/// unflushed writes. <see cref="LowMemoryMode"/> is the frugal extreme: the graph stays on disk and
/// is read through a small cache, whatever the budget would have allowed. Between them the index
/// picks its own residency: with room for everything it keeps the float vectors in memory too and a
/// search never touches the disk; with room for only the routing graph (the int8 vectors and the
/// edges, about a quarter of the float data) it walks in memory and reads floats only to re-score
/// the final candidates; with less than that it behaves as in <see cref="LowMemoryMode"/>.</para></summary>
public sealed class Hnsw2VectorIndexOptions {
    ...
    /// <summary>Trades speed for a minimal resident footprint: the graph stays on disk and is read
    /// through a small byte-budgeted cache, the upper layers are read from their file per hop, and
    /// <see cref="MaxMemoryBytes"/> defaults far lower. Searches whose working set exceeds the small
    /// cache pay a disk (or OS page cache) read per hop, and bulk loads become read-heavy, which is
    /// the price of the footprint. The files are identical in both modes, so an index written in one
    /// opens in the other; the mode is applied when the index opens.</summary>
    ...
    /// What does not fit stays on disk. Null resolves to 1 GB, or 32 MB in
    /// <see cref="LowMemoryMode"/>. A budget is a target rather than a hard wall — dirty state that
    /// is not yet flushed cannot be evicted — and it is adjustable at runtime through
    /// <see cref="Hnsw2VectorIndex.MaxMemoryBytes"/>.</summary>
    public long? MaxMemoryBytes { get; set; }
    internal long ResolvedMaxMemoryBytes => MaxMemoryBytes ?? (LowMemoryMode ? 32L * 1024 * 1024 : 1024L * 1024 * 1024);
    ...
    /// quarter of <see cref="MaxMemoryBytes"/>, capped at 64 MB (8 MB in
    /// <see cref="LowMemoryMode"/>).</summary>
    public long? MemTableFlushThresholdBytes { get; set; }
    internal long ResolvedMemTableFlushThresholdBytes =>
        MemTableFlushThresholdBytes ?? Math.Min(LowMemoryMode ? 8L * 1024 * 1024 : 64L * 1024 * 1024, Math.Max(1024 * 1024, ResolvedMaxMemoryBytes / 4));
    ...
    /// (<see cref="Hnsw2VectorIndex.AddRange"/>, and the WAL replay that uses it) keep the layer
