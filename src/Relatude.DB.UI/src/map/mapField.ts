 ...
 *                stops a dark ball on a dark page from being a hole in the page. Light ADDED to
 *                the page on a dark one, and laid OVER it on a light one, since nothing can be
 *                added to white (see airOverPage).
 *   the ground   a grid of quads over the whole world, which is a ball in the globe and a sheet on
 *                the flat map: the water. It also carries a raster of country NUMBERS sampled by
 *                longitude and latitude - which used to be how the land and the shaded countries
 *                were painted, and now serves the highlight as a mask telling water from rock,
 *                and the fallback should the land below not be built. Skipped on the flat map
 *                with nothing to draw.
 *   the land     every country as triangles cut from the very rings the coastlines are drawn
 *                from (see land.ts), lit exactly as the ground is and coloured by the country's
 *                number where the countries are shaded. The raster's cells are ten kilometres
 *                across and showed as blocks under a smooth coastline at any city's zoom; the
 *                triangles have the line's own edge at every zoom.
...
/**
 * And how far OUT. One is the whole ball in view with a little air round it, which is where a map
 * opens; below that the globe stands in the middle of the canvas as an object rather than filling
 * it, which is what you want when the marks on it are the point and the ground is not.
 */
export const minGlobeZoom = 0.4;
 ...
 * doing the moment it would put the camera inside the world. Below a zoom of one the same division
 * simply walks the camera backwards, and the near and far planes follow it out.
 */
function distanceOf(camera: GlobeCamera): number {
  return 1 + (fitDistance - 1) / Math.max(minGlobeZoom, Math.min(maxGlobeZoom, camera.zoom));
}
...
/**
 * How long an edge INSIDE a country may be before the land's triangles are cut smaller, in degrees.
 * Nothing is drawn along such an edge, so it need not match the lines; what bounds it is how far a
 * flat triangle sinks into the ball between its corners, which is what the cut along the edge of
 * the globe comes out wrong by: four degrees is under a pixel at any zoom that shows the edge.
 */
const maxInteriorDegrees = 4;
/**
...
/**
 * How strong the air is on a LIGHT page, as a share of what it is on a dark one.
 *
 * The two are not the same picture. On a dark page the air is light added to almost nothing, and a
 * pixel at the rim ends up the atmosphere's own colour. On a light page it has to be laid over the
 * page instead, and the very same alpha moves a white pixel most of the way to a saturated blue -
 * far further than the dark page ever moved, and it reads as a ring drawn round the globe rather
 * than as air. Held to about a third, it is a haze again.
 */
const airOnLightPage = 0.36;
...
export function createMapField(canvas: HTMLCanvasElement): MapField | null {
  ...
  const land = buildLandBuffer(gl);

  const groundProgram = program(gl, groundVert, surfaceFrag(false));
  const landProgram = program(gl, landVert, surfaceFrag(true));
  const lineProgram = program(gl, lineVert, lineFrag);
  const rodProgram = program(gl, rodVert, solidFrag);
  ...
  // the land: longitude, latitude and the country's number, three floats a vertex
  const landVao = gl.createVertexArray()!;
  gl.bindVertexArray(landVao);
  gl.bindBuffer(gl.ARRAY_BUFFER, land.vertices);
  attribute(gl, landProgram, "aPlace", 2, 0, 3, 0);
  attribute(gl, landProgram, "aCountry", 1, 0, 3, 2);
  gl.bindVertexArray(null);
  ...
  const pinBody = buffer(gl, gl.ARRAY_BUFFER, pinMesh());
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, groupBuffer);
  const pinGroup = gl.getAttribLocation(pinProgram, "aGroup");
  if (pinGroup >= 0) {
    gl.enableVertexAttribArray(pinGroup);
    gl.vertexAttribIPointer(pinGroup, 1, gl.UNSIGNED_SHORT, 0, 0);
  ...
  const surfaceRaster = texture(gl, gl.NEAREST, gl.REPEAT);
  ...
  /**
   * Whether the air has to be laid OVER the page rather than added to it: whether the page is
   * light. Adding light to white is the one thing that cannot show, so on the light theme the
   * halo outside the globe and the air at its limb both have to darken what is under them
   * instead - which is what haze against a bright sky does anyway.
   */
  function airOverPage(): boolean {
    const [r, g, b] = theme.clear;
  ...
  /**
   * What the ground and the land share: the light, the photographs, and what the land and the
   * water are painted with. The two are one fragment shader in two forms (see surfaceFrag), so the
   * same numbers go to both and a lit country matches the lit water round it.
   */
  function surface(p: WebGLProgram, scene: MapScene, painting: boolean) {
    const style = scene.style;
    place(p, scene);
    gl.uniform1i(gl.getUniformLocation(p, "uAirOver"), airOverPage() ? 1 : 0);
    gl.uniform3fv(gl.getUniformLocation(p, "uGround"), floats(theme.ground));
    gl.uniform3fv(gl.getUniformLocation(p, "uGlow"), floats(style.atmosphere > 0 ? style.atmosphereColor : theme.glow));
    gl.uniform3fv(gl.getUniformLocation(p, "uLight"), lightInWorld(scene));
    gl.uniform1f(gl.getUniformLocation(p, "uBrightness"), style.light.brightness);
    gl.uniform1f(gl.getUniformLocation(p, "uAmbient"), style.light.ambient);
    gl.uniform1f(gl.getUniformLocation(p, "uSpecular"), style.light.specular);
    gl.uniform1f(gl.getUniformLocation(p, "uShine"), style.light.shine);
    gl.uniform1f(gl.getUniformLocation(p, "uLandShine"), style.light.landShine);
    gl.uniform1i(gl.getUniformLocation(p, "uEarth"), style.earth === "off" ? 0 : style.earth === "day" ? 1 : style.earth === "night" ? 2 : style.earth === "moon" ? 4 : 3);
    gl.uniform1i(gl.getUniformLocation(p, "uHasDay"), hasDay ? 1 : 0);
    gl.uniform1i(gl.getUniformLocation(p, "uHasNight"), hasNight ? 1 : 0);
    gl.activeTexture(gl.TEXTURE2);
    gl.bindTexture(gl.TEXTURE_2D, dayMap);
    gl.uniform1i(gl.getUniformLocation(p, "uDay"), 2);
    gl.activeTexture(gl.TEXTURE3);
    gl.bindTexture(gl.TEXTURE_2D, nightMap);
    gl.uniform1i(gl.getUniformLocation(p, "uNight"), 3);
    gl.uniform1i(gl.getUniformLocation(p, "uPainted"), painting ? 1 : 0);
    gl.uniform1i(gl.getUniformLocation(p, "uHasLand"), style.land ? 1 : 0);
    gl.uniform3fv(gl.getUniformLocation(p, "uLandColor"), floats(style.landColor));
    gl.uniform3fv(gl.getUniformLocation(p, "uOceanColor"), floats(style.oceanColor));
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, surfaceColors);
    gl.uniform1i(gl.getUniformLocation(p, "uSurfaceColors"), 1);
  }
  ...
  return {
    ...
    setSurface(index, w, h) {
      hasSurface = true;
      gl.bindTexture(gl.TEXTURE_2D, surfaceRaster);
    ...
    draw(scene) {
      ...
      const style = scene.style;
      // The air round the globe is the whole canvas and goes behind everything else, so it may not
      // write to the depth buffer - a fullscreen pass sits halfway down the depth range and the
      // ball's near face can be further than that, so what it wrote would punch a hole in the
      // ground drawn over it.
      ...
      if (scene.globe && style.atmosphere > 0) {
        ...
        // On a dark page the air is light added to it. On a light one there is no light left to add
        // - white is where adding stops - so it is laid over the page instead, tinting it, and held
        // down to a haze (see airOnLightPage).
        const over = airOverPage();
        gl.blendFunc(gl.SRC_ALPHA, over ? gl.ONE_MINUS_SRC_ALPHA : gl.ONE);
        ...
        gl.uniform1f(gl.getUniformLocation(haloProgram, "uStrength"), over ? airOnLightPage : 1);
      ...
      // drawn only when there is something to put on it: the water under painted land, or the
      // shaded countries when there are no triangles to shade them with
      const painting = scene.surface && hasSurface;
      const vector = land.count > 0;
      gl.enable(gl.DEPTH_TEST);
      gl.depthMask(true);
      if (scene.globe || style.land || (painting && !vector)) {
        gl.enable(gl.BLEND);
        gl.blendFuncSeparate(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA, gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
        gl.useProgram(groundProgram);
        surface(groundProgram, scene, painting);
        gl.uniform1i(gl.getUniformLocation(groundProgram, "uVector"), vector ? 1 : 0);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, surfaceRaster);
      ...
      // the land, in its own colour or the shade of its country, with the coastline's own edge
      if (vector && (style.land || painting)) {
        gl.useProgram(landProgram);
        surface(landProgram, scene, painting);
        gl.bindVertexArray(landVao);
      ...
      if (scene.marks === "rods") {
        ...
        gl.uniform1f(gl.getUniformLocation(pinProgram, "uPinSize"), Math.max(1, scene.size * dpr));
        // what one device pixel is worth in the world a unit of depth away, which is what a pin is
        // scaled by (see pinVert) to keep the height it was asked for at every zoom
        gl.uniform1f(gl.getUniformLocation(pinProgram, "uPerPixel"), (2 * Math.tan(fov / 2)) / Math.max(1, canvas.height));
        // the globe's own light, so a pin agrees with the ball it stands on; flat, there is no ball
        ...
        gl.bindVertexArray(pinVao);
        gl.drawArraysInstanced(gl.TRIANGLES, 0, pinVertices, Math.min(pointCount, Math.max(1, scene.limit)));
        gl.disable(gl.DEPTH_TEST);
        ...
        gl.uniform1f(gl.getUniformLocation(markProgram, "uPointSize"), Math.max(1, scene.size * dpr));
...
/**
 * The land as triangles (see land.ts): three floats a vertex, three vertices a triangle. Should the
 * triangulation fail there are none, and the ground paints the land from its raster as it used to.
 */
function buildLandBuffer(gl: WebGL2RenderingContext) {
  try {
    const mesh = buildLand(maxSegmentDegrees, maxInteriorDegrees);
    return { vertices: buffer(gl, gl.ARRAY_BUFFER, mesh.vertices), count: mesh.vertexCount };
  } catch (error) {
    console.warn("map: the land could not be triangulated; painting it from the raster instead", error);
    return { vertices: buffer(gl, gl.ARRAY_BUFFER, new Float32Array(0)), count: 0 };
  }
}
...
/** How finely a pin is cut: how many sides round it, and how many rings up its head. */
const pinAround = 16;
const pinRings = 4;
/** How many vertices one pin is: a ring of triangles up the point, then the head's quads. */
const pinVertices = pinAround * 3 + pinAround * pinRings * 6;
...
/**
 * One pin, as a plain triangle list in a frame of its own: x and y across it, z from 0 at the POINT
 * - which is the place it stands on - to 1 at the top of its head.
 *
 * The shape is the one every map has used since paper: a round head on a cone tapering to a point.
 * The two meet where the cone is TANGENT to the head, which is the only arithmetic here and the
 * whole reason the thing reads as one object: joined anywhere else there is a crease, and a crease
 * catches the light as a ring of shadow round the neck. With a head of radius `r` whose middle sits
 * `c` up the pin, the cone touches it at z = c - r^2/c on a circle of radius r*sqrt(c^2 - r^2)/c,
 * and the cone's own normal leans out and DOWN by the same tangency.
 *
 * Convex, so nothing needs culling: a pin hides its own back through the depth buffer, as a rod does.
 */
function pinMesh(): Float32Array<ArrayBuffer> {
  const r = 0.3; // the head's radius, on a pin one unit tall
  const c = 0.7; // and how far up the pin its middle is, so that r + c is the top
  const meet = c - (r * r) / c; // where the point runs into the head
  const ring = (r * Math.sqrt(c * c - r * r)) / c; // and how wide it is there
  const out: number[] = [];
  const push = (x: number, y: number, z: number, n: [number, number, number]) => out.push(x, y, z, n[0], n[1], n[2]);
  const angle = (i: number) => ((i % pinAround) / pinAround) * Math.PI * 2;
  const coneNormal = (a: number): [number, number, number] => {
    const len = Math.hypot(meet, ring);
    return [(Math.cos(a) * meet) / len, (Math.sin(a) * meet) / len, -ring / len];
  };
  for (let i = 0; i < pinAround; i++) {
    const a0 = angle(i);
    const a1 = angle(i + 1);
    // the tip carries the normal of the face it belongs to, since one point serves every face
    push(0, 0, 0, coneNormal((a0 + a1) / 2));
    push(Math.cos(a0) * ring, Math.sin(a0) * ring, meet, coneNormal(a0));
    push(Math.cos(a1) * ring, Math.sin(a1) * ring, meet, coneNormal(a1));
  }
  // the head, in rings from where the cone left it up to the top; the normal of a sphere is the way
  // out of its middle, shared between the faces round a corner, which is what keeps it round
  const reach = Math.acos(-r / c); // how far off the top the tangent circle lies
  const corner = (p: number, a: number): { at: [number, number, number]; n: [number, number, number] } => {
    const n: [number, number, number] = [Math.sin(p) * Math.cos(a), Math.sin(p) * Math.sin(a), Math.cos(p)];
    return { at: [n[0] * r, n[1] * r, c + n[2] * r], n };
  };
  for (let k = 0; k < pinRings; k++) {
    const below = reach * (1 - k / pinRings);
    const above = reach * (1 - (k + 1) / pinRings);
    for (let i = 0; i < pinAround; i++) {
      const a0 = angle(i);
      const a1 = angle(i + 1);
      const q = [corner(below, a0), corner(below, a1), corner(above, a1), corner(above, a0)];
      for (const at of [0, 1, 2, 0, 2, 3]) push(q[at].at[0], q[at].at[1], q[at].at[2], q[at].n);
    }
  }
  const mesh = new Float32Array(out.length);
  mesh.set(out);
...
/**
 * The land, through the same projection as the ground it lies on. On the globe each vertex also
 * carries how far round towards the eye it is: a point of the ball is on the near side when
 * dot(p, eye - p) > 0, and with |p| = 1 that is dot(p, eye) > 1 - LINEAR in p, so it interpolates
 * across a flat triangle and the far side can be cut off along the edge of the ball by the fragment
 * shader, which a flat triangle straddling that edge would otherwise be drawn right across.
 */
const landVert = `#version 300 es
  in vec2 aPlace;
  in float aCountry;
  out vec2 vPlace;
  out vec3 vWorld;
  out float vFacing;
  flat out float vCountry;
${placeGlsl}
  void main() {
    vPlace = aPlace;
    vCountry = aCountry;
...
/**
 * The ground, lit - and the land, lit the same way, which is why this is one shader in two forms
 * rather than two shaders: the ground form reads the country under each fragment out of the raster,
 * the land form is told it by the triangle it is drawing, and everything after that is the same
 * text, so a country can never come out lit differently from the water round it.
 *
 ...
 * With the land drawn as triangles the ground is water everywhere (`uVector`), and its raster is
 * kept only as the mask the highlight wants - since the land pass lays its own, duller highlight
 * over the land, the two agree without the raster ever showing.
 *
 ...
 */
const surfaceFrag = (landPass: boolean) => `#version 300 es
  precision highp float;
  in vec2 vPlace;
  in vec3 vWorld;
  ${landPass ? "in float vFacing;\n  flat in float vCountry;" : "uniform bool uVector;"}
  uniform bool uGlobe;
  uniform vec3 uEye;
  ...
  uniform bool uAirOver;       // the page is light, so the air tints what is under it rather than lighting it
...
${
  landPass
    ? `    // the far side of the ball, cut off along its edge with a pixel's fade rather than a step
    float facing = uGlobe ? clamp(vFacing / max(fwidth(vFacing), 1e-7) + 0.5, 0.0, 1.0) : 1.0;
    if (facing <= 0.002) discard;
    // this triangle IS land, of the country it was cut from; the palette holds what that is painted with
    float land = 1.0;
    float fill = 1.0;
    vec4 painted = uPainted ? texelFetch(uSurfaceColors, ivec2(int(vCountry + 0.5), 0), 0) : vec4(0.0);`
    : `    // the raster holds a country's NUMBER; the palette holds what that number is painted with.
    // With the land drawn over the ground as triangles, the raster is a mask for the shine and no
    // more: the ground is then water wherever it shows, and the countries are painted on the land
    float facing = 1.0;
    ...
    float fill = uVector ? 0.0 : land;
    vec4 painted = (uPainted && !uVector) ? texelFetch(uSurfaceColors, ivec2(int(country * 255.0 + 0.5), 0), 0) : vec4(0.0);`
}

    // what the ball is made of, before any light falls on it
    vec3 base = uHasLand ? mix(uOceanColor, uLandColor, fill) : uGround;
    ...
    // and on the ball the land is drawn only where it is asked for too; the ground under it is always there
    float alpha = ${landPass ? "(uHasLand ? 1.0 : painted.a) * facing" : "1.0"};
    ...
    // the air seen THROUGH, at the limb where there is most of it to see through. Added to a dark
    // ball as light, and laid over a light one as colour, for the same reason the halo outside the
    // globe is (see airOverPage): nothing can be added to white. The two meet at the ball's edge
    // with about the same weight, so the air carries on across it rather than stopping at it.
    float rim = pow(1.0 - max(0.0, dot(n, toEye)), 3.0);
    color = uAirOver ? mix(color, uGlow, rim * 0.30) : color + uGlow * rim * 0.30;
...
/**
 * What both solids on the map - the rods and the pins - come out of. Nothing is left to do here:
 * each was shaded at the vertex it was built from, and its edges are the canvas's own.
 */
const solidFrag = `#version 300 es
  precision highp float;
  in vec4 vColor;
  out vec4 outColor;
...
/**
 * A pin: a solid standing on its place, along the line from the MIDDLE OF THE EARTH out through it.
 * That line is the whole idea - a pin marks a spot on a ball, so it leans with the ground it stands
 * on, and a ring of them round the globe's edge fans outwards the way a hedgehog's spines do rather
 * than all pointing up the page.
 *
 * Its size is in PIXELS and not in the world, which is the one way it differs from a rod. A rod
 * stands for a patch of ground and grows and shrinks with the Earth; a pin is a marker, meant to be
 * looked at and clicked, and a marker that halves when you zoom out is no use. So the pin is scaled
 * by how far its foot is from the eye - `uPerPixel` is what one device pixel is worth out there at
 * unit depth - which leaves it the same height on the screen at every zoom.
 *
 * Drawn with the depth buffer the ground has already written into: that hides every pin on the far
 * side of the ball, sorts pin against pin, and hides each pin's own back faces, a teardrop being
 * convex. The facing test is still made, and on the foot rather than on the vertex, so a pin just
 * over the horizon goes away as one thing rather than sinking into the ground head first.
 */
const pinVert = `#version 300 es
  in vec3 aPin;      // across the pin (x, y), and along it (0 at the point, 1 at the top of the head)
  in vec3 aNormal;   // the surface's own normal, in that same frame
  in vec2 aPlace;    // per pin: where it stands
  in uint aGroup;    // per pin: which of the palette's colours it wears
  uniform float uLift;
  uniform float uPinSize;   // how tall a pin is, in device pixels
  uniform float uPerPixel;  // and what one device pixel is worth in the world, a unit of depth away
  uniform vec3 uLight;
...
${placeGlsl}
  /**
   * How bright a point of a pin is. The light is wrapped round the terminator rather than cut off
   * at it, so the side facing away is dim and not black - a pin carries a colour that means
   * something - and a tight highlight is laid on top, which is what makes the head read as a bead
   * rather than as a flat blob with a gradient on it.
   */
  vec4 shade(vec3 color, vec3 n, vec3 toEye) {
    float lit = clamp(dot(n, uLight) * 0.5 + 0.5, 0.0, 1.0);
    float gloss = pow(max(0.0, dot(n, normalize(uLight + toEye))), 26.0) * 0.45;
    return vec4(clamp(color * (0.46 + 0.72 * lit * lit) + vec3(gloss), 0.0, 1.0), 1.0);
  }
  ...
  void main() {
    ...
    if (uGlobe) {
      ...
      vec3 foot = axis * uLift;
      vec3 toFoot = normalize(uEye - foot);
      if (dot(axis, toFoot) <= 0.0) { gl_Position = vec4(2.0, 2.0, 2.0, 1.0); return; }  // round the far side
      // as tall on the screen from far off as from close to: what a pixel is worth out where the
      // pin stands is that distance times what it is worth at one
      float size = uPinSize * uPerPixel * max(0.0001, (uViewProj * vec4(foot, 1.0)).w);
      // a frame square to the pin; which way round it lies does not matter, only that it is square
      vec3 side = normalize(cross(axis, abs(axis.y) > 0.99 ? vec3(1.0, 0.0, 0.0) : vec3(0.0, 1.0, 0.0)));
      vec3 other = cross(axis, side);
      vec3 at = foot + (side * aPin.x + other * aPin.y + axis * aPin.z) * size;
      vColor = shade(color, normalize(side * aNormal.x + other * aNormal.y + axis * aNormal.z), normalize(uEye - at));
      gl_Position = uViewProj * vec4(at, 1.0);
      return;
    }
    // Flat, the pin stands up the page and is looked at from straight on, so the axis across it
    // that points at the viewer shows only in the shading and in the depth - as a rod's does.
    vec2 footPx;
    placeOf(aPlace, uLift, footPx);
    vec2 px = footPx + vec2(aPin.x, -aPin.z) * uPinSize;
    // screen y grows DOWN the page, so the normal's does too on the way into the light
    vColor = shade(color, normalize(vec3(aNormal.x, aNormal.z, aNormal.y)), vec3(0.0, 0.0, 1.0));
    // a depth of its own, as the rods have: in front of the flat ground at 0, and a pin whose foot
    // is further down the page - nearer the viewer, the page being read as a floor - in front of
    // the ones above it
    float depth = -0.08 - clamp(footPx.y / uSize.y, 0.0, 1.0) * 0.9 - aPin.y * 0.008;
    gl_Position = vec4(toClip(px, uSize).xy, depth, 1.0);
  }`;
 ...
 *
 * `uStrength` is how much of it is wanted, which is all of it when the page is dark enough for the
 * air to be added to it and a third when it has to tint a light page instead; the blend mode that
 * goes with each is chosen by the caller.
 */
const haloFrag = `#version 300 es
  ...
  void main() {
    ...
    float glow = pow(1.0 - out_, 3.0) * uStrength;
    if (glow <= 0.004) discard;
    outColor = vec4(uColor, glow);
  }`;

/**
 * One node, as a point sprite on its place: the dots, and the same sprite counted rather than drawn
 * for the heat field. `uScale` and `uTarget` are what let one shader do both - the field is counted
 * at a fraction of the canvas. The pins went off to a mesh of their own (see pinVert).
 */
