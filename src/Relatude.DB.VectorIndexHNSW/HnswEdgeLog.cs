...
/// <summary>
/// The neighbour lists changed since the last state save, as a sequential append-only log.
///
...
/// thousands of existing records scattered all over the graph file. Writing them in place at every
...
/// page back first. Measured on a 50,000 × 1536 index that turned a 500-vector delta into roughly
/// 400 ms of checkpoint. Appending the same edges to this log instead is one sequential write of a
...
/// the log holds edges the graph file has not received, the record store keeps those lists in an overlay
/// and stamps them onto any record it loads from the file, so a reader never sees the stale copy. A
/// state save writes them into the graph file, stamps a manifest claiming zero entries, and only then
/// drops the log — so a crash anywhere in that sequence either replays entries that are already applied
/// (they are idempotent) or ignores a log the manifest no longer claims.</para>
/// </summary>
internal sealed class HnswEdgeLog : IDisposable {
    internal const int FileKind = 4;
    readonly FixedStrideFile _file;
    readonly int _capacity; // neighbour slots per entry, i.e. the layer-0 degree
    readonly int _words;    // [ordinal][count][ordinals: capacity]
    ...
    HnswEdgeLog(FixedStrideFile file, int capacity) {
        _file = file;
        _capacity = capacity;
        _words = 2 + capacity;
    }
    public static HnswEdgeLog Create(string path, long generation, int neighbourCapacity) {
        var words = 2 + neighbourCapacity;
        return new(FixedStrideFile.Create(path, FileKind, generation, words * 4, [neighbourCapacity], 0), neighbourCapacity);
    }
    public static HnswEdgeLog Open(string path, long generation, int neighbourCapacity, int committedEntries) {
        var words = 2 + neighbourCapacity;
    ...
    /// <summary>Buffers one node's new neighbour list. Nothing reaches the file until
    /// <see cref="FlushPending"/>, which writes the whole batch in one go.</summary>
    public void Append(int ordinal, ReadOnlySpan<int> neighbours) {
        if (neighbours.Length > _capacity) throw new ArgumentException("More neighbours than the log entry has slots for. ");
        _pending.Add(ordinal);
        _pending.Add(neighbours.Length);
        foreach (var n in neighbours) _pending.Add(n);
        for (var i = neighbours.Length; i < _capacity; i++) _pending.Add(0);
    }
    ...
    /// ones, so a replay must apply them in this order.</summary>
    public IEnumerable<(int ordinal, int[] neighbours)> Replay(int count) {
        ...
        for (var first = 0; first < count; first += perBatch) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                yield return (ordinal, buffer.AsSpan(at + 2, length).ToArray());
            }
        }
    }
    /// <summary>Forgets the entries without touching the file: their edges have been written into the
    /// graph file, so the next manifest can claim none of them. The bytes are still there — harmlessly,
