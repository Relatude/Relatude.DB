...
/// 400 ms of checkpoint. Appending the same regions to this log instead is one sequential write of a
...
/// the log holds regions the graph file has not received, the record store keeps them in an overlay
...
internal sealed class HnswEdgeLog : IDisposable {
    ...
    /// ones, so a replay must apply them in this order. Each yields the raw edge region the record
