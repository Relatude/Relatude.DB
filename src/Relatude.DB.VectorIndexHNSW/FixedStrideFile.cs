...
/// <summary>
/// An array of fixed-size records on disk, addressed by index. All three files of the graph are one
/// of these: the graph records, the node table, and the upper-layer adjacency.
///
/// <para>Unlike the IVF index's immutable segments these files are updated in place — a graph insert
/// rewrites the neighbour lists of existing nodes, so there is nothing immutable to swap. What makes
/// that safe is that the index only ever <i>appends</i> record indexes: the manifest's record count
/// is the commit boundary, and anything beyond it is uncommitted scratch the next open ignores.
/// A record below the boundary can be rewritten (a neighbour list changing), and a write torn by a
/// crash then leaves a mix of old and new neighbour ids — all of them valid ids either way, so the
/// worst case is a node with slightly worse edges, never a lost or corrupted vector.</para>
///
...
internal sealed class FixedStrideFile : IDisposable {
    ...
    const long Magic = 0x3146_5453_5648_5452; // "RTHVSTF1"
    const int Version = 1;
