...
/// <summary>
/// The index's durable root: which generation of the three graph files is live, how many records of
/// each of them are committed, where a search enters the graph, the WAL file the data belongs to and
/// the timestamp of the last durable position (<see cref="HnswVectorIndex.PersistedTimestamp"/>).
...
/// </summary>
internal sealed class HnswManifest {
    const long Magic = 0x314D_5648_4244_5452; // "RTDBHVM1", changes with breaking format changes
    const int Version = 1;
    ...
    public long Generation;      // which set of graph_/nodes_/upper_ files is live
    public int NextOrdinal;      // committed record count of the graph and node files
    ...
    /// <summary>Durable entries of the edge log — neighbour lists the graph file has not received yet.
    /// Anything the log holds beyond this was not durable when the manifest was written, so it is not
    /// replayed. See <see cref="HnswEdgeLog"/>.</summary>
    public int EdgeLogEntries;

    public static HnswManifest? TryRead(string path) {
        try {
            ...
            var pos = 12;
            var m = new HnswManifest { WalFileId = new Guid(bytes.AsSpan(pos, 16)) };
            pos += 16;
...
internal sealed class HnswPaths(string folder) {
    ...
    public string Graph(long generation) => Path.Combine(folder, "graph_" + generation.ToString("d8") + ".bin");
    ...
    public string[] Generation(long generation) => [Graph(generation), Nodes(generation), Upper(generation), Edges(generation)];
    /// <summary>Every data file in the folder, whichever generation it belongs to.</summary>
    public IEnumerable<string> AllDataFiles() =>
        Directory.GetFiles(folder, "graph_*.bin")
