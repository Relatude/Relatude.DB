...
/// <summary>
/// The index's durable root: which segments are live, the WAL file the data belongs to, and the
/// timestamp of the last durable position (<see cref="IIndex.PersistedTimestamp"/>). Written via a
/// temp file and an atomic replace, and only after the segments it references are fsynced — a crash
/// mid-write leaves the previous manifest in place, never a half-written one. A missing, corrupt or
/// foreign-WAL manifest resets the index to empty so the WAL replay rebuilds it.
/// </summary>
internal sealed class TextIndexManifest {
    const long Magic = 0x3146_4E4D_5854_4452; // arbitrary marker, changes with breaking format changes
    const int Version = 1;
    public Guid WalFileId;
    public long Timestamp;
    public long NextSegmentId = 1;
    public long[] SegmentIds = [];
    public static TextIndexManifest? TryRead(string path) {
        try {
            if (!File.Exists(path)) return null;
            var bytes = File.ReadAllBytes(path);
            if (bytes.Length < 8 + 4 + 16 + 8 + 8 + 4 + 8) return null;
    ...
    public void Write(string path) {
        var bytes = new byte[8 + 4 + 16 + 8 + 8 + 4 + SegmentIds.Length * 8 + 8];
