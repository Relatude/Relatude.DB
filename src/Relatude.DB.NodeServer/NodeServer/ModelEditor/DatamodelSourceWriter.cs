...
///
/// The exception is a compiled source whose code folder is generated
/// (<see cref="DatamodelSource.GenerateModelFile"/>): that folder is the editor's, and the plan
/// makes it hold exactly the generated files and nothing else - see <see cref="planGeneratedFolder"/>.
/// </summary>
public static class DatamodelSourceWriter {
    ...
    public static (bool writable, string? reason, bool requiresRebuild) Writability(DatamodelSource source, string rootFolder) {
        switch (source.Type) {
            ...
            case DatamodelSourceType.TypeReference: {
                    ...
                    if (!source.GenerateModelFile && !Directory.Exists(folder)) return (false, "The source code folder \"" + folder + "\" does not exist.", false);
    ...
    public static SourceWritePlan Plan(Datamodel active, Datamodel draft, string rootFolder, Func<Guid, IIOProvider?> resolveIO) {
        ...
        foreach (var source in sources) {
            ...
            var change = new SourceChange {
                ...
                Generated = IsGeneratedFolder(source),
            ...
            // a generated folder is planned at every activation, model changes or not: a file somebody
            // dropped into it since the last one has to go, and a folder that was never generated fills up
            if (!change.HasModelChanges && !change.Generated) continue;
            if (!writable) {
    ...
    /// up holding exactly one file per node type and relation of the source, named after it (by full name
    /// when two share a name), each starting with <see cref="ModelGen.AutoGeneratedHeader"/>. Every other
    /// file in the folder, however deep, is deleted. A file that does not carry the generated-code marker
    /// was written by hand, so deleting or overwriting it is reported as a warning the activation needs
    /// accepted - the plan is what the user sees before saying yes. Stamped file names are ignored: the
    /// layout is the generator's.
    /// </summary>
    static void planGeneratedFolder(SourceWritePlan plan, DatamodelSource source, SourceChange change, Datamodel draft,
        List<NodeTypeModel> dTypes, List<RelationModel> dRelations, string rootFolder) {
        var folder = DatamodelSourceLoader.ResolveSourceCodeFolder(source, rootFolder)!;
        var members = dTypes.Select(t => (id: t.Id, isType: true, code: t.CodeName, full: t.FullName))
            .Concat(dRelations.Select(r => (id: r.Id, isType: false, code: r.CodeName, full: r.FullName()))).ToList();
        var shared = members.GroupBy(m => m.code, StringComparer.OrdinalIgnoreCase).Where(g => g.Count() > 1).Select(g => g.Key).ToHashSet(StringComparer.OrdinalIgnoreCase);
        var generated = new Dictionary<string, (List<Guid> types, List<Guid> relations)>(StringComparer.OrdinalIgnoreCase);
        foreach (var m in members) {
            var file = (shared.Contains(m.code) ? m.full : m.code) + ".cs";
            if (!generated.TryGetValue(file, out var bucket)) generated[file] = bucket = ([], []);
            (m.isType ? bucket.types : bucket.relations).Add(m.id);
        }
        var existing = Directory.Exists(folder)
            ? Directory.GetFiles(folder, "*", SearchOption.AllDirectories).Select(f => normalize(Path.GetRelativePath(folder, f))).ToList()
            : [];
        var handWritten = new List<string>();
        static bool isGenerated(string path) {
            try { return ModelGen.IsAutoGenerated(File.ReadAllText(path)); } catch { return false; } // unreadable or binary: not ours
        }
        var header = ModelGen.AutoGeneratedHeader(source.Name);
        foreach (var (relative, (typeIds, relationIds)) in generated.OrderBy(kv => kv.Key, StringComparer.OrdinalIgnoreCase)) {
            var typeSet = typeIds.ToHashSet();
            var relationSet = relationIds.ToHashSet();
            var content = header + ModelGen.GenerateCSharpModelCode(draft, true, t => typeSet.Contains(t.Id), r => relationSet.Contains(r.Id));
            var path = Path.Combine(folder, relative);
            var exists = File.Exists(path);
            var byHand = exists && !isGenerated(path);
            if (byHand) handWritten.Add(relative);
            plan.Files.Add(new PlannedFile {
                SourceId = source.Id, Path = path, RelativePath = relative, Action = PlannedFileAction.Write,
                Content = content, Exists = exists, Changed = !exists || !sameText(File.ReadAllText(path), content), HandWritten = byHand,
                NodeTypeIds = typeIds, RelationIds = relationIds,
            });
        }
        foreach (var relative in existing.Where(f => !generated.ContainsKey(f)).OrderBy(f => f, StringComparer.OrdinalIgnoreCase)) {
            var path = Path.Combine(folder, relative);
            var byHand = !isGenerated(path);
            if (byHand) handWritten.Add(relative);
            plan.Files.Add(new PlannedFile {
                SourceId = source.Id, Path = path, RelativePath = relative, Action = PlannedFileAction.Delete, Exists = true, Changed = true, HandWritten = byHand,
            });
        }
        if (handWritten.Count > 0) {
            var listed = string.Join(", ", handWritten.Take(10)) + (handWritten.Count > 10 ? " and " + (handWritten.Count - 10) + " more" : "");
            plan.Issues.Add(DatamodelIssue.Warning("hand-written-files", "The source code folder of \"" + change.Name + "\" (" + folder + ") is generated as a whole when the model is activated, and it holds "
                + handWritten.Count + " file" + (handWritten.Count == 1 ? "" : "s") + " that " + (handWritten.Count == 1 ? "was" : "were") + " not generated by Relatude.DB: " + listed
                + ". Activating deletes " + (handWritten.Count == 1 ? "it" : "them") + ". Move anything worth keeping out of the folder first, or turn off \"Generate source code\" on the source.", source: source.Id));
        }
    }
    ...
    static void planFiles(SourceWritePlan plan, DatamodelSource source, SourceChange change, Datamodel draft,
        ...
        if (IsGeneratedFolder(source)) {
            planGeneratedFolder(plan, source, change, draft, dTypes, dRelations, rootFolder);
            return;
        }
