...
/// locations, and old backups stay where they were taken.
/// </summary>
[TestClass]
public class FolderLayoutTests {
    ...
    public void FileKeys_UseTheFolderLayout() {
        ...
        Assert.AreEqual("data/db.00000001.bin", keys.WAL_GetFileKey(1));
        Assert.AreEqual("data/db.log", keys.WAL_GetSecondaryFileKey());
        Assert.AreEqual("state/state.bin", keys.StateFileKey);
        Assert.AreEqual("state/index.abc.bin", keys.Index_GetFileKey("abc"));
        Assert.AreEqual("state/mapper.5.dll", keys.MapperDll_GetFileKey(5));
        Assert.AreEqual("state/queue.bin", keys.Queue_GetFileKey("bin"));
        var dt = new DateTime(2026, 8, 23, 10, 30, 0, DateTimeKind.Utc);
        Assert.AreEqual("bkup/db.2026-08-23-10-30-00.bkup", keys.WAL_GetFileKeyForBackup(dt, false));
        Assert.AreEqual("bkup/db.bkup.keep.2026-08-23-10-30-00.bkup", keys.WAL_GetFileKeyForBackup(dt, true));
        Assert.AreEqual("bkup/files.2026-08-23-10-30-00.bkup", keys.FileStore_GetFileKeyForBackup(dt, false));
        Assert.AreEqual("log/critical.error.txt", keys.CriticalErrorLogFileKey);
        Assert.AreEqual("indexes/ai.cache.bin", keys.GetAiCacheFileKey(AIProviderCacheType.Sqlite));
        Assert.AreEqual("indexes/native.ai.cache.bin", keys.GetAiCacheFileKey(AIProviderCacheType.Native));
    ...
    public void FileKeys_KeepThePrefixOnTheFileNameInsideTheFolder() {
        ...
        Assert.AreEqual("data/abc.db.00000001.bin", keys.WAL_GetFileKey(1));
        Assert.AreEqual("state/abc.state.bin", keys.StateFileKey);
        Assert.AreEqual("bkup/abc.db.2026-08-23-10-30-00.bkup", keys.WAL_GetFileKeyForBackup(new DateTime(2026, 8, 23, 10, 30, 0, DateTimeKind.Utc), false));
        // the indexes folder itself is prefixed, so the ai cache file inside it is not
        Assert.AreEqual("abc.indexes/ai.cache.bin", keys.GetAiCacheFileKey(AIProviderCacheType.Sqlite));
    }

    [TestMethod]
    public void ValidateFileKeyString_AcceptsFolderQualifiedKeysAndRejectsEscapes() {
        FileKeyUtility.ValidateFileKeyString("db.00000001.bin");
        FileKeyUtility.ValidateFileKeyString("data/db.00000001.bin");
        foreach (var bad in new[] { "", "/x", "x/", "data//x", "../x", "data/../x", "data/./x" }) {
            Assert.ThrowsException<ArgumentException>(() => FileKeyUtility.ValidateFileKeyString(bad), "must reject: " + bad);
        }
    }
    ...
    public void GetAllFileKeys_ListsOnlyThisInstancesFiles() {
        ...
        var prefixed = new FileKeyUtility("abc").GetAllFileKeys(io);
    ...
    public void HotSwappedLogFlaggedWithALegacyKey_IsNotDeleted() {
        ...
        var io = new IOProviderMemory();
        var count = insertArticlesAndSimulateLegacyLayout(io, out var legacyKey, out var newKey);
        io.WriteString("rewrite.flag", legacyKey);
        openAndAssertIntact(io, count);
    ...
    class NoRenameIOProvider(IIOProvider inner) : IIOProvider {
        public IReadStream OpenRead(string fileKey, long position) => inner.OpenRead(fileKey, position);
        public IReadStream OpenRead(string[] path, long position) => inner.OpenRead(path, position);
        public IAppendStream OpenAppend(string fileKey) => inner.OpenAppend(fileKey);
        public IAppendStream OpenAppend(string[] path) => inner.OpenAppend(path);
        public bool Exists(string fileKey) => inner.Exists(fileKey);
        public bool Exists(string[] path) => inner.Exists(path);
        public bool DoesNotExistOrIsEmpty(string fileKey) => inner.DoesNotExistOrIsEmpty(fileKey);
        public void DeleteFileIfItExists(string[] path) => inner.DeleteFileIfItExists(path);
        public void DeleteFileIfItExists(string fileKey) => inner.DeleteFileIfItExists(fileKey);
        public FileMeta[] GetFiles() => inner.GetFiles();
        public long GetFileSizeOrZeroIfUnknown(string file) => inner.GetFileSizeOrZeroIfUnknown(file);
        public long GetFileSizeOrZeroIfUnknown(string[] path) => inner.GetFileSizeOrZeroIfUnknown(path);
        public bool CanRenameFile => false;
        public void RenameFile(string fileKey, string newFileKey) => throw new NotSupportedException();
        public bool CanTruncate => inner.CanTruncate;
        public void TruncateFile(string fileKey, long newLength) => inner.TruncateFile(fileKey, newLength);
