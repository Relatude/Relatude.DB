...
When the user wants a **new, empty Relatude.DB project** — a web app, a website, an API, "set up Relatude.DB", "scaffold", "hello world" — do not hand-write the csproj, `Program.cs`, `relatude.db.json`, views or a client from memory. The command line tool generates a verified starting point in one command, in one of several **project types**:

```bash
dotnet tool install -g Relatude.DB.Tool            # once; "dotnet tool update -g Relatude.DB.Tool" upgrades it
relatude new --list-types                          # the project types, with what each is suited for
relatude new MyApp                                 # default type csharp_web_react_ts: Backend/ + Client/
...
2. **Ask which project type the user wants**, unless the request already makes it clear. Run `relatude new --list-types` (`--json` for a machine-readable list) and present each type in a sentence or two. Today there are two:
   ...
   When the request says "app", "dashboard" or "interactive", propose the SPA type; when it says "website", "content", "blog", "pages" or "SEO", propose the MVC type — and confirm before generating. Types for other languages and stacks will join the list under the same `language_platform_flavour` naming, so read the list instead of assuming these two.
3. **Run `relatude new <Name> --projecttype <type>`** from the folder that should contain the project (the type may be omitted for the default). `--out <folder>` puts it elsewhere, `--user <name> --password <pw>` sets the admin UI login (optional: on localhost the admin UI needs none), `--force` writes into a non-empty folder, `--json` returns the file list and the next steps as JSON. The name becomes the folder, the C# root namespace (PascalCased, so `my-app` is `MyApp`) and the model namespace `<Name>.Models`.
4. **Read the generated `README.md`.** It says how to run the project (SPA: `dotnet run --project Backend --launch-profile https` and `npm install --prefix Client && npm run dev --prefix Client`, then <http://localhost:5173>; MVC: `dotnet run --launch-profile https`, then <https://localhost:7238>), where models go (the `Models` folder of the C# project, namespace `<Name>.Models`, picked up through `relatude.db.json` with no registration code — and only node types belong there, since every class in that namespace becomes one), and where pages or endpoints go (`Controllers/` plus `Views/` for MVC; `Backend/Program.cs` with `RelatudeDBContext` injected plus `Client/src/api.ts` for the SPA).
5. **Build before handing over** — `dotnet build` on the C# project, plus `npm install` and `npm run build` in `Client` for the SPA type — and use the tool on the result: `relatude validate` and `relatude schema` from the folder holding `relatude.db.json` read the model without opening the database; `relatude info` and `relatude query "<Type>.Count()"` open it, so stop the app first (single writer).

Both types generate a net10.0 project referencing `Relatude.DB.Server` plus the Skia and FFMpeg plugins at the tool's own version (`--package-version` overrides it; keep tool and packages on the same version, the settings file uses the vocabulary of the tool's version), `AddRelatudeDB` / `UseRelatudeDB` in `Program.cs`, a middleware serving stored files by URL, `relatude.db.json` with fresh ids, native index engines, a file store and the model namespace, `Models/README.md` with a sample node class, and a `.gitignore` that keeps build output and the database folder `relatude.db/` out of git. The SPA type adds `/api/hello`, an SPA fallback and the Vite client with a dev proxy for `/api` and `/relatude.db`; the MVC type adds `HomeController`, a `_Layout.cshtml` that writes `<title>` and `<meta name="description">` from `ViewData`, an error page and `wwwroot/css/site.css`, and its README shows a products controller with list, details and search pages.

Two related commands: `relatude init --namespace <Ns>` writes only the `relatude.db.json` into an existing project, and `relatude help all` prints the whole CLI reference. When the user wants something no type fits — a console app, a library, an existing solution — skip `new` and follow `references/setup.md`; `relatude init` still writes the settings file.
...
10. **Start new projects with `relatude new <Name> --projecttype <type>`**, installing `Relatude.DB.Tool` first when `relatude version` fails, and ask the user which project type fits before generating — an interactive application (React SPA, `csharp_web_react_ts`) or a content website with traditional navigation and SEO (server-rendered MVC, `csharp_web_mvc`); `relatude new --list-types` has the current list (see "Starting a new project"). Never scaffold a Relatude.DB web app by hand when the tool can generate a verified one; build it and run `relatude validate` before handing it over.
