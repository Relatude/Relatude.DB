...
.NET 10+. The engine runs in-process or server-hosted. `Relatude.DB.Server` is the package that ships the server and the admin UI; plugin packages exist for Azure, Lucene and Sqlite (`Relatude.DB.Plugins.Azure` / `.Lucene` / `.Sqlite`).
